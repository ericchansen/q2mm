...
RE_ENERGY = r"(\s|\*)Conformation\s+\d+\s\(\s+(?P<energy>[\d\.\-]+)\s+kJ/mol\) " r"was found\s+(?P<num>[\d]+)"
HARTREE_TO_KJMOL = 2625.5  # Hartree to kJ/mol


def read_energy_from_macro_log(filename, max_structures=None, max_energy=None):
    energies = []
...
def return_parser():
    ...
    parser.add_argument(
        "-g", "--group", metavar="filename", type=str, nargs="+", action="append", help="Group of filenames."
    )
    parser.add_argument(
        "-n",
        "--max_structures",
    ...
    parser.add_argument(
        "-a", "--appendfile", metavar="filename", type=str, help="Also append CSV style output to a file."
    )
    parser.add_argument(
        "-t", "--temperature", type=float, default=298.15, help="Self-explanatory. Default is 298.15 K."
    )
    return parser
...
def sumq(groups, appendfile=None, max_energy=None, max_structures=None, temperature=298.15):
    # list of lists
        ...
        for filename in group:
            if filename.endswith(".log"):
                ...
                try:
                    ...
                    e = read_energy_from_macro_log(filename, max_structures=max_structures, max_energy=max_energy)
...
def main(args):
    ...
    sumq(opts.group, opts.appendfile, opts.max_energy, opts.max_structures, opts.temperature)

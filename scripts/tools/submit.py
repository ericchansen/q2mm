...
def CRC_qsub(job_name, USER, QUEUE, CPU, COMMAND):
    ...
    submission_file.write(DEFAULT_SUB_FILE.format(job_name, USER, QUEUE, CPU, COMMAND.format(job_name + ".com")))
...
def return_parser():
    parser = argparse.ArgumentParser(description="To fill out later")
    parser.add_argument("filename", type=str, nargs="+", help="Filename")
    parser.add_argument("-q", "--queue", type=str, help='"long" or "debug"')
    parser.add_argument(
        "-pe",
        "--processors",
        type=str,
        ...
        const="none",
        default="default",
        help="No option string = default smp 8; \n"
    ...
    parser.add_argument(
        ...
        "--command",
        type=str,
        help="Command that are being ran. The \
    ...
    parser.add_argument(
        ...
        "--username",
        type=str,
        help="Notre Dame CRC user name. Probably \
            a NetID.",
    )
    return parser

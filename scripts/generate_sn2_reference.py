...
Run with: conda run -n q2mm python scripts/generate_sn2_reference.py
"""
...
def checkout_resource_dir(script_path: Path = Path(__file__)) -> Path:
    ...
    if not (repository / "pyproject.toml").is_file() or not (resource_dir / "manifest.json").is_file():
        raise RuntimeError(
            "SN2 reference regeneration must run from the checked-out scripts/generate_sn2_reference.py script."
        )
    return resource_dir


OUTPUT_DIR = checkout_resource_dir()
LOG_PATH = REPO_ROOT / "examples" / "ch3f-sn2" / "psi4-output.dat"

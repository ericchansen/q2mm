...
class GaussCom:
    ...
    def write_com(self, directory="./"):
        ...
        with open(directory + self.filename + ".com", "w") as f:
            ...
            if self.ECP:
                route_section = ["#", "empiricaldispersion=gd3", "int=ultrafine", "genecp", self.method]
            else:
                route_section = ["#", "empiricaldispersion=gd3", "int=ultrafine", self.basis, self.method]
            if self.frequency:
            ...
            for i, atom in enumerate(self.atom_list):
                f.write(
                    "   ".join((f" {atom[0]:2}", f"{atom[1]:14.10f}", f"{atom[2]:14.10f}", f"{atom[3]:14.10f}", "\n"))
                )
            f.write("\n")
    ...
    def get_frozen_coords(self, dict_of_patterns, from_command_line=False):
        ...
        if from_command_line:
            dict_of_patterns = self.get_dictionary_of_frozen_coords(dict_of_patterns)
        for pattern in dict_of_patterns:
            matches = analyze.evaluate_substructure(self.mae_struct, pattern, first_match_only=False)
            for atom_indicies in matches:
                str_indicies = []
                for index in atom_indicies:
                    str_indicies.append(str(index))
                frozen_coord_lines.append("{} {} F".format(dict_of_patterns[pattern], " ".join(str_indicies)))
...
def main(args):
    ...
    if opts.calculationtype == "FZTS" and not opts.frozenatoms:
        raise Exception("The FZTS argument can only be used when frozen atoms" + " are indicated.")
    for filename in opts.filename:
        ...
        for i, structure in enumerate(structures):
            ...
            if len(structures) == 1:
                ...
                file_iterator_name = os.path.splitext(filename)[0] + f"_{i + 1:03d}"
                gaussian_file = GaussCom(
                    filename=file_iterator_name,
                    atom_list=structures[structure][1],
                    calculation_type=opts.calculationtype,
                    charge=structures[structure][0],
                    mae_struct=structure,
            ...
            if opts.frozenatoms:
                gaussian_file.frozen_atoms = gaussian_file.get_frozen_coords(opts.frozenatoms, from_command_line=True)
            if opts.basisset:
...
def return_parser():
    parser = argparse.ArgumentParser(
        description="Automatically make a \

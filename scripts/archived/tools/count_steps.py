...
RE_INPUT = r"\s+Search initialized with\s+(?P<num>\d+)\s+structures from the " "input structure file"
RE_TOTAL = r"\s+Total number of structures processed =\s+(?P<num>\d+)"


def count_steps(direc):
    filenames = glob.glob(os.path.join(direc, "*.log"))
    filenames.sort()
    mcmm_steps = 0
    lmcs_steps = 0
    lmc2_steps = 0
...
def main(args):
    ...
    parser.add_argument(
        "-a",
        action="store_true",
        help="Search all sub directories for conformational search log files. Still displays each diretory separately.",
    )
    parser.add_argument(
        "-d",
        type=str,
        help="Search the provided directory for conformational search log files and count the number of steps.",
    )

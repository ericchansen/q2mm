...
def return_compare_parser():
    ...
    parser.add_argument(
        ...
        help=(
            "These commands produce the FF data. Leave one space after the 1st quotation mark enclosing the arguments."
        ),
    )
    parser.add_argument(
        "--reference",
        ...
        metavar='" commands for calculate.py"',
        help=(
            "These commands produce the QM/reference data. Leave one space "
            "after the 1st quotation mark enclosing the arguments."
        ),
    )
    parser.add_argument("--output", "-o", type=str, metavar="filename", help="Write pretty output to filename.")
    parser.add_argument("--print", "-p", action="store_true", dest="print", help="Print pretty output.")
    return parser
...
def compare_data_old(r_data, c_data):
    ...
    *** opting in for a dictionary of data types.
    ...
    assert len(r_data) == len(c_data), "Length of reference data and FF data doesn't match!"

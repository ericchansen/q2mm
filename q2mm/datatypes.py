            ...
            for i, line in enumerate(f):
                ...
                if gather_data and split:
                    ...
                    if "ANGL" in line and count == 2:
                        ...
                        self.params.extend(
                            (
                                ParamMM3(atom_types=at, ptype="bf", ff_col=1, ff_row=i + 1, value=float(BB[0])),
                                ParamMM3(atom_types=at, ptype="be", ff_col=2, ff_row=i + 1, value=float(BB[1])),
                            )
                        )
                    # ANGLE
                    if "DIHE" in line and count == 3:
                        ...
                        self.params.extend(
                            (
                                ParamMM3(atom_types=at, ptype="af", ff_col=1, ff_row=i + 1, value=float(BB[0])),
                                ParamMM3(atom_types=at, ptype="ae", ff_col=2, ff_row=i + 1, value=float(BB[1])),
                            )
                        )
                    # Dihedral
                    if "IMPR" in line and count == 4:
                        ...
                        self.params.append(
                            ParamMM3(atom_types=at, ptype="df", ff_col=1, ff_row=i + 1, value=float(BB[1]))
                        )
                    ...
                    if "NONB" in line and count == 5:
                        ...
                        at = [AA[0], AA[1], AA[2], AA[3]]
                        self.params.append(
                            ParamMM3(atom_types=at, ptype="imp1", ff_col=1, ff_row=i + 1, value=float(BB[0]))
                        )
        ...
        with open(path) as f:
            ...
            for i, line in enumerate(f):
                ...
                if "OPT" in line and section_vdw:
                    ...
                    self.params.extend(
                        (
                            ParamMM3(atom_types=atm, ptype="vdwr", ff_col=1, ff_row=i + 1, value=float(rad)),
                            ParamMM3(atom_types=atm, ptype="vdwe", ff_col=2, ff_row=i + 1, value=float(eps)),
                        )
                    )
                    continue
                if "OPT" in line or section_sub:
                    ...
                    # Higher order torsions.
                        ...
                        logger.log(5, "[L{}] Found higher order torsion:\n{}".format(i + 1, line.strip("\n")))

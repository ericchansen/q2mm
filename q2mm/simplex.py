    ...
    def run(self, r_data=None):
        ...
        if self.max_params and len(self.ff.params) > self.max_params:
            ...
            if None in [x.d1 for x in self.ff.params]:
                ...
                logger.log(5, f"  -- Keeping {len(ffs)} forward differentiated FFs.")
            else:
            ...
            params = select_simp_params_on_derivs(self.ff.params, max_params=self.max_params)
        ...
        while current_cycle < self.max_cycles and cycles_wo_change < self._max_cycles_wo_change:
            ...
            logger.log(20, wrapper.fill("{}".format(" ".join(f"{x.score:15.4f}" for x in self.new_ffs))))
            ...
            if self.do_weighted_reflection:
                ...
                score_diff_sum = sum([x.score - self.new_ffs[-1].score for x in self.new_ffs[:-1]])
                if score_diff_sum == 0.0:
                    logger.warning("No difference between force field scores. Exiting simplex.")
                    # We want to raise opt.OptError such that
                    # opt.catch_run_errors will write the best FF obtained thus
                    # far.
                    raise opt.OptError("No difference between force field scores. Exiting simplex.")
            for i in range(0, len(last_best_ff.params)):
                if self.do_weighted_reflection:
                    inv_val = (
                        sum([x.params[i].value * (x.score - self.new_ffs[-1].score) for x in self.new_ffs[:-1]])
                        / score_diff_sum
                    )
                else:
                    inv_val = sum([x.params[i].value for x in self.new_ffs[:-1]]) / len(self.new_ffs[:-1])
            ...
            if ref_ff.score < last_best_ff.score:
                ...
                for i in range(0, len(last_best_ff.params)):
                    exp_ff.params[i].value = 3 * inv_ff.params[i].value - 2 * self.new_ffs[-1].params[i].value
                ...
                if exp_ff.score < ref_ff.score:
                    ...
                    logger.log(20, "  -- Expansion succeeded. Keeping expanded parameters.")
                else:
                    self.new_ffs[-1] = ref_ff
                    logger.log(20, "  -- Expansion failed. Keeping reflected parameters.")
                ...
                for i in range(0, len(last_best_ff.params)):
                    if ref_ff.score > self.new_ffs[-1].score:
                        con_val = (inv_ff.params[i].value + self.new_ffs[-1].params[i].value) / 2
                    else:
                        con_val = (3 * inv_ff.params[i].value - self.new_ffs[-1].params[i].value) / 2
                ...
                if con_ff.score < self.new_ffs[-2].score:
                    ...
                    for ff_num, ff in enumerate(self.new_ffs[1:]):
                        for i in range(0, len(last_best_ff.params)):
                            ff.params[i].value = (ff.params[i].value + self.new_ffs[0].params[i].value) / 2
                    ...
                    logger.log(20, "  -- Contraction failed. Keeping parmaeters anyway.")
                    self.new_ffs[-1] = con_ff
            ...
            if self.new_ffs[0].score < last_best_ff.score:
                ...
                logger.log(
                    20,
                    f"  -- {cycles_wo_change} cycles without improvement out of {self._max_cycles_wo_change} allowed.",
                )
            logger.log(20, "BEST:")
...
def calc_simp_var(params):
    ...
    for param in params:
        param.simp_var = param.d2 / param.d1**2.0
    ...
    if len(old_ff.params) > len(new_ff.params):
        ...
        for i, param_o in enumerate(old_ff.params):
            for param_n in new_params:
                ...
                if param_o.ff_row == param_n.ff_row and param_o.ff_col == param_n.ff_col:

...
class Loop:
    ...
    def opt_loop(self):
        ...
        while last_score is None or change is None or change > self.convergence:
            self.cycle_num += 1
        ...
        while True:
            ...
            if cols[0] == "FFLD":
                ...
                if cols[1] == "read":
                    ...
                    if "prm" in line:
                        self.ff = datatypes.TinkerFF(os.path.join(self.direc, cols[2]))
                    if "frcmod" in line:
                        self.ff = datatypes.AmberFF(os.path.join(self.direc, cols[2]))
                    self.ff.import_ff()
                    self.ff.method = "READ"
            ...
            if cols[0] == "PARM":
                ...
                self.ff.params = parameters.trim_params_by_file(self.ff.params, os.path.join(self.direc, cols[1]))
            if cols[0] == "LOOP":
                ...
                # Log commands.
                pretty_loop_input(inner_loop_lines, name="OPTIMIZATION LOOP", score=self.ff.score)
                # Run inner loop.
            ...
            if cols[0] == "COMP":
                ...
                if "-p" in cols:
                    doprint = True
                self.ff.score = compare.compare_data(r_dict, c_dict, output=output, doprint=doprint)
            if cols[0] == "GRAD":
                grad = gradient.Gradient(direc=self.direc, ff=self.ff, ff_lines=self.ff.lines, args_ff=self.args_ff)
                ...
                for col in cols[1:]:
                    if "lstsq" in col:
                        ...
                        for arg in g_args:
                            ...
                            if "radii" in arg:
                                grad.lstsq_radii = []
                                radii_vals = re.search(r"\[(.+)\]", arg).group(1).split("/")
                                if radii_vals == "None":
                            ...
                            if "cutoff" in arg:
                                grad.lstsq_cutoff = []
                                cutoff_vals = re.search(r"\[(.+)\]", arg).group(1).split("/")
                                if cutoff_vals == "None":
                                    grad.lstsq_cutoff = None
                                else:
                                    if len(cutoff_vals) > 2 or len(cutoff_vals) < 2:
                                        raise Exception("Cutoff values must be between two numbers.")
                        ...
                        for arg in g_args:
                            ...
                            if "factor" in arg:
                                grad.levenberg_cutoff = []
                                factor_vals = re.search(r"\[(.+)\]", arg).group(1).split("/")
                                if factor_vals == "None":
            ...
            if cols[0] == "SIMP":
                simp = simplex.Simplex(direc=self.direc, ff=self.ff, ff_lines=self.ff.lines, args_ff=self.args_ff)
                for col in cols[1:]:
...
def read_loop_input(filename):
    with open(filename) as f:
        lines = f.readlines()
    lines = [x.partition("#")[0].strip("\n") for x in lines if x.partition("#")[0].strip("\n") != ""]
...
def main(args):
    ...
    parser.add_argument("input", type=str, help="Filename containing loop commands.")

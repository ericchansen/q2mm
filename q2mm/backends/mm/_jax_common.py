...
def compute_param_offsets(
    ...
    """Compute parameter vector offsets for bond/angle/torsion/vdw/ub blocks.

    The parameter vector layout is:
      ``[bond_k, bond_r0, ..., angle_k, angle_theta0, ..., torsion_k, ...,
      vdw_radius, vdw_eps, ..., ub_k, ub_eq, ...]``

    Each bond type contributes 2 values (k, r0), each angle type 2
    (k, theta0), each torsion type 1 (k), each vdW type 2
    (radius, epsilon), and each UB type 2 (k, eq).
    ...
    Returns:
        ...
        ``"ub"`` mapping to the starting index of each block in the flat
        parameter vector.

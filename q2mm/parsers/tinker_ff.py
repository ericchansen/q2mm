...
# Keyword lists for Tinker parameter types.
...
class _TinkerBase(FF):
    """Shared base for Tinker parameter file readers.

    Provides the common ``__init__``, lazy ``lines`` property, and
    ``import_ff`` used identically by both ``TinkerFF`` and ``TinkerMM3A``.
    Subclasses only need to implement ``export_ff``.
    """

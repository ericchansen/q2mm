...
class ScipyOptimizer:
    ...
    def _run_minimize(
        ...
        # Revert guard: if the run worsened the objective, restore x0.
        if use_evaluator_gradient and final_score > initial_score:
            logger.warning(
                "Optimization worsened the objective: %.6g -> %.6g. Reverting to initial parameters.",
                initial_score,
                final_score,
            )
            final_x = x0.copy()
        ...
        return final_x, final_score, nit, bool(scipy_result.success), message

...
def _score_interval_summary(
    initial: Sequence[float], final: Sequence[float], *, executor: Literal["python", "jax"]
) -> dict[str, Any]:
    ...
    return {
        ...
        "optimizer_improvement_pct_mean": improvement,
        "optimizer_improvement_significant": bool(abs(final_mean - initial_mean) > (initial_ci95 + final_ci95)),
...
def _data_provenance(case: BenchmarkCase, resolved_roots: Mapping[str, str]) -> dict[str, Any]:
    ...
    return {
        "scientific_problem": {
            ...
            "input_fingerprints": dict(problem_input_fingerprints(problem)),
        },
...
def _execute(
    *,
    ...
    summary.update(_score_interval_summary(result.initial_samples, result.final_samples, executor=executor_kind))

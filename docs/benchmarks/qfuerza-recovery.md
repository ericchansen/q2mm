...
| Frozen vs active partition (`opt_substructure_membership` + `ActiveParameterSpace.from_membership`) | Literature `.fld` | Chemistry decision: which parameters can be re-derived per-system |
...
Both files are round-tripped through `q2mm.io.mm3.save_mm3_fld()` first so

...
class TestMethodD:
    ...
    def test_returns_copy(self, ts_eigenvalues: np.ndarray) -> None:
    ...
    def test_preserves_negative(self, ts_eigenvalues: np.ndarray) -> None:
        result = keep_natural_eigenvalue(ts_eigenvalues)
        assert result[0] < 0
        assert result[0] == ts_eigenvalues[0]

    def test_all_positive_unchanged(self, positive_eigenvalues: np.ndarray) -> None:
        result = keep_natural_eigenvalue(positive_eigenvalues)
        np.testing.assert_array_equal(result, positive_eigenvalues)

    def test_multi_neg_preserved(self, multi_neg_eigenvalues: np.ndarray) -> None:
...
class TestInvertTSCurvature:
    ...
    def test_method_c_no_negative_eigenvalues(
        self, synthetic_ts_hessian: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
    ...
    def test_method_d_preserves_negative_eigenvalue(
        self, synthetic_ts_hessian: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
    ...
    def test_method_d_is_identity_transform(
        self, synthetic_ts_hessian: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
    ...
    def test_default_is_method_c(self, synthetic_ts_hessian: tuple[np.ndarray, np.ndarray, np.ndarray]) -> None:
        H, _, _ = synthetic_ts_hessian
    ...
    def test_method_c_replaces_with_large_positive(
        self, synthetic_ts_hessian: tuple[np.ndarray, np.ndarray, np.ndarray]
    ) -> None:
        H, _, _ = synthetic_ts_hessian
        result = invert_ts_curvature(H, method="C")
...
class TestDetectProblematicParams:
    ...
    def _make_ff(self, bond_fcs: list[float], angle_fcs: list[float]) -> ForceField:
    ...
    def test_negative_bond_fc(self) -> None:
        ff = self._make_ff([359.7, -7.2], [43.2, 57.6])
        result = detect_problematic_params(ff)
        assert ("C", "H") in result["bonds"]
        assert result["angles"] == set()

    def test_zero_angle_fc(self) -> None:
    ...
    def test_all_problematic(self) -> None:
...
class TestLockParams:
    ...
    def _make_ff(self, bond_fc: float, bond_eq: float, angle_fc: float, angle_eq: float) -> ForceField:
        return ForceField(
            name="test",
    ...
    def test_lock_preserves_unlocked(self) -> None:
    ...
    def test_lock_empty_keys(self) -> None:
        target = self._make_ff(5.0, 1.09, 0.6, 109.5)
        source = self._make_ff(215.8, 1.10, 50.4, 108.0)
...
class TestMethodDOnSN2:
    ...
    def sn2_hessian(self) -> np.ndarray:
        return np.load(str(SN2_HESSIAN))

    def test_sn2_has_negative_eigenvalues(self, sn2_hessian: np.ndarray) -> None:
    ...
    def test_sn2_most_negative_is_reaction_coordinate(self, sn2_hessian: np.ndarray) -> None:
    ...
    def test_method_c_removes_negative(self, sn2_hessian: np.ndarray) -> None:
        result = invert_ts_curvature(sn2_hessian, method="C")
        evals = np.linalg.eigvalsh(result)
        assert np.all(evals >= -1e-10)

    def test_method_d_preserves_negative(self, sn2_hessian: np.ndarray) -> None:
        result = invert_ts_curvature(sn2_hessian, method="D")
        evals = np.linalg.eigvalsh(result)
        assert np.any(evals < 0)

    def test_method_d_eigenvalues_match_original(self, sn2_hessian: np.ndarray) -> None:
    ...
    def test_method_c_vs_d_most_negative_differs(self, sn2_hessian: np.ndarray) -> None:

...
def test_canonical_union_annotation_does_not_enable_lossy_typed_saving() -> None:
    value: ObservationValue = ObservationSet().with_atomic_partial_charge(0.25, atom_index=1, case_id="h2").values[0]
    with pytest.raises(reference.ReferenceYAMLError, match="atomic_partial_charge.*h2.*metadata is not represented"):
        reference._reference_value_to_dict(value)

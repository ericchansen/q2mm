...
class TestScientificProblemIdentity:
    @pytest.mark.parametrize("operation", ["input-only", "benchmark", "sdk"])
    def test_fingerprints_build_the_complete_payload_once(
        self, scientific_case: BenchmarkCase, monkeypatch: pytest.MonkeyPatch, operation: str
    ) -> None:
        ...
        problem = _problem() if operation == "sdk" else scientific_case.problem
        expected_fingerprint = models.problem_fingerprint(problem)
        expected_inputs = {
            f"molecule:{case.case_id}": canonical_fingerprint(
                models.molecule_fingerprint_payload(case.molecule), screen_secrets=True
            )
            for case in problem.cases
        }
        expected_inputs["starting_force_field"] = canonical_fingerprint(
            models.force_field_fingerprint_payload(
                problem.starting_force_field, problem.layout.vector(problem.starting_force_field)
            ),
            screen_secrets=True,
        )
        expected_payload = models.problem_fingerprint_payload(problem)
        for key in ("observations", "active_space"):
            expected_inputs[key] = canonical_fingerprint(expected_payload[key])

        calls: list[int] = []
        original = models.problem_fingerprint_payload

        def counted_payload(value: Any) -> dict[str, Any]:
            calls.append(id(value))
            return original(value)

        monkeypatch.setattr(models, "problem_fingerprint_payload", counted_payload)
        if operation == "input-only":
            assert models.problem_input_fingerprints(problem) == expected_inputs
        elif operation == "benchmark":
            identity = _data_provenance(scientific_case, {})["scientific_problem"]
            assert identity["fingerprint"] == expected_fingerprint
            assert identity["input_fingerprints"] == expected_inputs
        else:
            ...
            monkeypatch.setattr(
                ...
                lambda *args, **kwargs: (
                    result,
                    problem.layout.replace(problem.starting_force_field, result.final_params),
                ),
            )
            run = optimize(
                problem,
                _EnergyBackend(),
                recipe="explicit",
                optimizer=ScipyOptimizer(method="Nelder-Mead", maxiter=0),
                workflow="single-stage",
                executor="python",
                gradient_mode="none",
                n_evals=0,
            )
            assert run.problem_fingerprint == expected_fingerprint
            assert run.input_fingerprints == expected_inputs
        assert calls == [id(problem)]
    ...
    def test_sdk_identity_failure_never_falls_back_to_metadata(
        ...
        def reject_identity(_problem: Any) -> dict[str, Any]:
            raise CanonicalizationError("Secret-like field is not permitted.")

        monkeypatch.setattr("q2mm.application.models.problem_fingerprint_payload", reject_identity)

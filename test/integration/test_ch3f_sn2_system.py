...
class TestCh3fSn2System:
    ...
    def test_system_is_registered(self) -> None:
        """``ch3f-sn2`` appears in the benchmark registry with TS metadata."""
        from q2mm.benchmarks.systems import SYSTEM_KEYS, system_metadata
        from q2mm.benchmarks.systems.ch3f_sn2 import KEY, METADATA, NAME

        assert KEY in SYSTEM_KEYS
        spec = system_metadata(KEY)
        assert spec.name == NAME
        assert METADATA["is_transition_state"] is True
        assert "Limé" in METADATA["publication"] or "Lime" in METADATA["publication"]
        assert METADATA["doi"] == "10.1002/jcc.23797"

    def test_loader_returns_one_molecule_with_hessian(self) -> None:
        """Loader produces one molecule with the packaged TS Hessian.
        ...
        """
        from q2mm.benchmarks.systems.ch3f_sn2 import load_molecule
    ...
    def test_geometry_has_two_c_f_bonds(self) -> None:
        ...
        The default ``load_xyz`` ``bond_tolerance`` of
        ...
        assert len(cf_bonds) == 2, (
            ...
            "Check the ``bond_tolerance`` in ``load_molecule``."
        )
    ...
    def test_load_system_succeeds_with_default_kwargs(self) -> None:
        """``load_system('ch3f-sn2')`` produces a usable BenchmarkCase.
        ...
        assert len(case.qm_freqs_per_mol[0]) >= 8, (
            ...
            f"got {len(case.qm_freqs_per_mol[0])}: {case.qm_freqs_per_mol[0]}"
        )
    ...
    def test_qfuerza_with_small_replace_with_produces_positive_facaf(self) -> None:
        ...
        facaf_large = [
            a
            for a in case_large.problem.starting_force_field.angles
            if set(a.elements) == {"F", "C"} and a.elements[1] == "C"
        ]
        facaf_small = [
            a
            for a in case_small.problem.starting_force_field.angles
            if set(a.elements) == {"F", "C"} and a.elements[1] == "C"
        ]

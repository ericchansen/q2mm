...
def _assert_preflight_failure(component: Any, kind: str, monkeypatch: pytest.MonkeyPatch) -> None:
    calls = []
    ...
    with pytest.raises(ApplicationConfigurationError, match="configuration|ConfigurationProvider|JSON"):
        optimize(
            _problem(),
            backend,
            recipe="explicit",

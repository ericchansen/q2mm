...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("jaxopt-lbfgs", "jaxopt:lbfgs"),
        ("basinhopping", "basinhopping"),
        ("multi-lbfgsb-5", "multi:L-BFGS-B"),
    ],
)
def test_catalog_and_cycling_delegate_constructor_choice_to_one_owner(
    key: str, method: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    from q2mm.optimizers import catalog

    plan = catalog._leaf(ScipyOptimizer, "scipy", method="Powell", maxiter=7, verbose=False)
    decisions = []

    def shared(selected: str, options: Mapping[str, object], **kwargs: object) -> catalog._Construction:
        decisions.append((selected, kwargs.get("syntax", "catalog")))
        return plan

    monkeypatch.setattr(catalog, "_optimizer_construction", shared)
    optimizer, recorded = resolve_optimizer(key)
    loop = _loop(method)
    full = loop._build_full_optimizer()
    assert decisions == [(method, "catalog"), (method, "cycling"), ("Nelder-Mead", "scipy")]
    assert type(optimizer) is type(full) is ScipyOptimizer
    assert _view(optimizer) == _view(full) == _view(plan.build())
    assert recorded == plan.settings(OPTIMIZER_CATALOG[key].gradient_mode)
